...
export default function Palette1Page() {
  ...
  async function executeQuery(qText, source = 'text') {
    ...
    try {
      if (USE_MOCKS) {
        ...
        const mockData = isOffTopic ? { ...refusedMock } : { ...successMock };
      ...
      const res = await fetch(`${API_URL}/api/query`, {
        ...
        body: JSON.stringify({ question: q, source, chunking_strategy: strategy }),
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error([data.message || data.error || 'Query failed', data.stage ? `(${data.stage})` : ''].filter(Boolean).join(' '));
      }
      setResult(data);
      const total = data.latency_ms?.total;
      setStatusMsg(typeof total === 'number' ? `🔥 MASTERED IN ${total.toFixed(1)}MS` : '🔥 GROUNDED ANSWER READY');
    } catch (err) {
  ...
  const toggleMic = () => {
    setStatusMsg('🎙️ Use the main GOA EMERALD stage for live mic → /api/stt, or type a text query here.');
                  ...
                  <div className="beach-strategy-faders">
                    {STRATEGIES.map((st) => (
                      <button
                        ...
                        onClick={() => {
                          setStrategy(st);
                          if (question.trim()) executeQuery(question, 'text');
                        }}
            ...
            {result && (
              <div className="beach-grounded-plate p1-plate">
                <div className="beach-plate-header">
                  ...
                  <span>TOTAL LATENCY: {totalMs ? `${totalMs.toFixed(1)}MS` : '—'}</span>
                </div>
